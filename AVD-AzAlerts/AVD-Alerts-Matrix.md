...
- **Severity override:** Pass `-Severity 0` (Critical) or `-Severity 2` (Warning) to `Azure-AVD-Alerts.ps1` to override the default for all alerts.
...
- **Adding new error codes:** Add `CodeSymbolic` values to the appropriate category's KQL `where` clause in the `$alertDefinitions` array in `Azure-AVD-Alerts.ps1`.
