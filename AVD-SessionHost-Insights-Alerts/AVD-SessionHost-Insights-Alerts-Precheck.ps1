...
<#
.SYNOPSIS
    Validates prerequisites for AVD Session Host Insights Alerts deployment.
...
.DESCRIPTION
    ...
    to the workspace before running AVD-SessionHost-Insights-Category-Alerts.ps1.
...
if (-not $CsvPath) {
    $CsvPath = ".\avd-sessionhost-insights-precheck-$($SubscriptionId.Substring(0,8)).csv"
}
...
if ($azCliAvailable) {
    ...
    Write-Log ""
    Write-Log "Azure CLI is required for all remaining checks. Install Azure CLI and re-run." "Red"
    try {
        ...
        Write-Log "Report exported to: $CsvPath" "Yellow"
    } catch {
...
if ($LASTEXITCODE -ne 0 -or [string]::IsNullOrWhiteSpace($roleAssignmentsJson)) {
    ...
    # LAW role can be granted directly on the workspace resource scope.
    $workspaceScope = "/subscriptions/$SubscriptionId/resourceGroups/$WorkspaceResourceGroupName/providers/Microsoft.OperationalInsights/workspaces/$WorkspaceName"
    $lawScopeRoleIds = @()
    ...
    if ($hasFullRole) {
        ...
        foreach ($req in $requiredActions) {
            $reqScope = if ($req.Action -eq "Microsoft.OperationalInsights/workspaces/read") { $workspaceScope } else { $rgScope }
            Add-CheckResult -Check $req.Action -Scope $reqScope -Result "PASS" -Detail "Covered by '$matchedRole'"
        }
    } elseif ($hasMonitoringContrib -and $hasLAWRole) {
        foreach ($req in $requiredActions) {
            $reqScope = if ($req.Action -eq "Microsoft.OperationalInsights/workspaces/read") { $workspaceScope } else { $rgScope }
            Add-CheckResult -Check $req.Action -Scope $reqScope -Result "PASS" -Detail "Covered by Monitoring Contributor + LAW role (RG or workspace scope)"
        }
    } else {
        ...
        if (-not $hasLAWRole) {
            Add-CheckResult -Check "Microsoft.OperationalInsights/workspaces/read" -Scope $workspaceScope -Result "FAIL" -Detail "Missing. Assign 'Log Analytics Reader'"
        } else {
            Add-CheckResult -Check "Microsoft.OperationalInsights/workspaces/read" -Scope $workspaceScope -Result "PASS" -Detail "Covered by LAW role"
        }
    }
...
if ($failCount -gt 0) {
    Write-Log ""
    Write-Log "One or more checks failed. Resolve the issues above before running AVD-SessionHost-Insights-Category-Alerts.ps1" "Red"
    exit 1
