...
3. Or override any parameter:
  .\AVD-SessionHost-Insights-Category-Alerts.ps1 -SubscriptionId "12345678-..." `
     -ResourceGroup "rg-avd" -WorkspaceName "law-avd" -Location "eastus2" `
     -WebhookUrl "https://contoso.logic.azure.com/workflows/..."
==============================================================================
.SYNOPSIS
  Deploys AVD Session Host Insights performance and session lifecycle alerts from a config file.
...
.DESCRIPTION
  ...
  REQUIRED: A Log Analytics workspace receiving AVD Session Host Insights data (Perf, WVDCheckpoints,
...
.EXAMPLE
  .\AVD-SessionHost-Insights-Category-Alerts.ps1 -ResourceGroup "rg-avd-prod" `
    -WorkspaceName "law-avd-prod" -Location "eastus2"

.EXAMPLE
  .\AVD-SessionHost-Insights-Category-Alerts.ps1 -CategoryFilter "HostPerformance" -WhatIf

.EXAMPLE
  .\AVD-SessionHost-Insights-Category-Alerts.ps1 -AlertFilter "AVD-SessionHost-Insights-Category-HostPerformance" -Severity 1
#>
...
param(
  ...
  [Parameter(Mandatory = $false)]
  [ValidateNotNullOrEmpty()]
  [string]$ActionGroupName = "AVD-SessionHost-Insights-Detailed",

  [Parameter(Mandatory = $false)]
  [ValidateNotNullOrEmpty()]
  [string]$WebhookReceiverName = "AVDSessionHostInsightsDetailedWebhook",
...
if (-not $CsvPath) {
  if ($SubscriptionId) {
    $CsvPath = ".\avd-sessionhost-insights-report-$($SubscriptionId.Substring(0,8)).csv"
  } else {
    $CsvPath = ".\avd-sessionhost-insights-report.csv"
  }
}
...
$rbacWorkspaceRG = if ([string]::IsNullOrWhiteSpace($WorkspaceResourceGroupName)) { $ResourceGroup } else { $WorkspaceResourceGroupName }
$workspaceScope = "/subscriptions/$($accountInfo.id)/resourceGroups/$rbacWorkspaceRG/providers/Microsoft.OperationalInsights/workspaces/$WorkspaceName"
...
if ($LASTEXITCODE -ne 0 -or [string]::IsNullOrWhiteSpace($roleAssignmentsJson)) {
  ...
  # LAW role can be granted at LAW scope even when absent at deployment RG scope.
  $lawScopeRoleIds = @()
  $lawScopeAssignmentsJson = az role assignment list `
    --assignee $principalId `
    --scope $workspaceScope `
    --include-inherited `
    --include-groups `
    --output json 2>$null
  if ($LASTEXITCODE -eq 0 -and -not [string]::IsNullOrWhiteSpace($lawScopeAssignmentsJson)) {
    $lawScopeAssignments = $lawScopeAssignmentsJson | ConvertFrom-Json
    $lawScopeRoleIds = $lawScopeAssignments | ForEach-Object { ($_.roleDefinitionId -split '/')[-1] }
  }
  ...
  $hasLAWContribOrReader =
    ($assignedRoleIds -contains $logAnalyticsContribId) -or
    ($assignedRoleIds -contains $logAnalyticsReaderId) -or
    ($lawScopeRoleIds -contains $logAnalyticsContribId) -or
    ($lawScopeRoleIds -contains $logAnalyticsReaderId)
  ...
  if ($hasFullRole) {
    ...
    Write-Host "[Pre-flight] RBAC OK - 'Monitoring Contributor' + Log Analytics role cover permissions (RG or LAW scope)." -ForegroundColor Green
  } else {
...
foreach ($alert in $allAlerts) {
  ...
  Write-Progress -Activity 'Deploying AVD Session Host Insights Alerts' `
  ...
  $operation = if ($alertExists -and -not $CreateOnly) { "Update" } else { "Create" }
  $processAction = if ($operation -eq "Update") {
    "Recreate scheduled query alert to apply updated query/settings"
  } else {
    "Create scheduled query alert"
  }

  if ($PSCmdlet.ShouldProcess($alert.Name, $processAction)) {
    Write-Log "$operation`: $($alert.Name) [$($alert.Category)] (Severity: $severityText)" "Cyan"
    ...
    try {
      ...
      if ($operation -eq "Update") {
        $deleteOutput = az monitor scheduled-query delete `
          -g $ResourceGroup -n $alert.Name `
          --subscription $script:subscriptionId --yes 2>&1
        if ($LASTEXITCODE -ne 0) {
          throw "Failed to delete existing alert '$($alert.Name)' before recreate: $deleteOutput"
        }
      }
      ...
      if ($LASTEXITCODE -eq 0) {
        if ($operation -eq "Update") {
          Write-Log "  [OK] Updated successfully." "Green"
          $UpdatedAlerts += $alert.Name
        } else {
        ...
        $AlertResults += [pscustomobject]@{
          ...
          Description = $alert.Description
          Severity    = "$severityVal ($severityText)"
          Action      = if ($operation -eq "Update") { "Updated" } else { "Created" }
          Status      = "Success"
        }
      } else {
        ...
        if ($errStr -match "conflict|already exists") {
          if ($operation -eq "Update") {
            Write-Log "  [FAIL] Conflict while recreating existing alert: $output" "Red"
            $FailedAlerts += $alert.Name
            $AlertResults += [pscustomobject]@{
              AlertName   = $alert.Name
              Category    = $alert.Category
    ...
    if ($operation -eq "Update") {
      Write-Log "[WhatIf] Would update: $($alert.Name)" "Yellow"
    } else {
    ...
    $AlertResults += [pscustomobject]@{
      ...
      Description = $alert.Description
      Severity    = "$severityVal ($severityText)"
      Action      = if ($operation -eq "Update") { "WouldUpdate" } else { "WouldCreate" }
      Status      = "WhatIf"
    }
  }
}

Write-Progress -Activity 'Deploying AVD Session Host Insights Alerts' -Completed
...
# ----------------------------
Write-Log ""
Write-Log "=== AVD Session Host Insights Alerts Summary ===" "Cyan"
...
if ($whatIfCount -gt 0) {
  $wouldCreateCount = ($AlertResults | Where-Object Action -eq "WouldCreate").Count
  $wouldUpdateCount = ($AlertResults | Where-Object Action -eq "WouldUpdate").Count
  $wouldSkipCount   = ($AlertResults | Where-Object Action -eq "Skipped").Count
  Write-Log "WhatIf Mode: $wouldCreateCount create, $wouldUpdateCount update, $wouldSkipCount skipped" "Yellow"
} else {
  Write-Log "=== Statistics ===" "Cyan"
  Write-Log "Created:  $($NewlyCreated.Count)" "Green"
  Write-Log "Updated:  $($UpdatedAlerts.Count)" "Green"
...
$AlertResults | Group-Object Category | ForEach-Object {
  ...
  $updated = ($_.Group | Where-Object Action -eq "Updated").Count
  $skipped = ($_.Group | Where-Object Action -eq "Skipped").Count
  $failed  = ($_.Group | Where-Object { $_.Action -in @("Failed","Error") }).Count
  Write-Log "  $($_.Name): $($_.Count) total ($created created, $updated updated, $skipped skipped, $failed failed)" "Gray"
}
