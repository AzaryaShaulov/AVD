...
==============================================================================
.SYNOPSIS
    Deploys and configures the AVD Session Host Insights Alerts Logic App webhook notification pipeline.

.DESCRIPTION
    Creates or updates the Logic App workflow, ensures the AVD-SessionHost-Insights-Detailed
    webhook action group, validates and authorizes required API connections, and
    configures alert routing so session host insights alerts send detailed
...
.PARAMETER LogicAppName
    Name of the Logic App workflow used for detailed AVD Session Host Insights Alerts notifications.
...
.NOTES
    ...
    - Deploys/updates Logic App workflow resources used for detailed session host insights notifications.
    - Ensures AVD-SessionHost-Insights-Detailed action group webhook receiver is present and points to callback URL.
    - Assigns Log Analytics Reader to the Logic App managed identity for query access.
    - Verifies required session host insights alerts exist and bootstraps them via AVD-SessionHost-Insights-Category-Alerts.ps1 when missing.
    - Applies detailed-only routing for session host insights alerts after webhook deployment.
...
param(
    ...
    [string]$DetailedActionGroupName = "AVD-SessionHost-Insights-Detailed",
    [string]$DetailedWebhookReceiverName = "AVDSessionHostInsightsDetailedWebhook",
    [string]$CsvPath = "",
    [hashtable]$Tags = @{},
...
$HardCoded = @{
    ...
    SendFromEmail               = ""
    Office365ConnectionName     = "avd-alerts-office365"
    DetailedActionGroupName     = "AVD-SessionHost-Insights-Detailed"
    DetailedWebhookReceiverName = "AVDSessionHostInsightsDetailedWebhook"
    Tags                        = @{
        Solution    = "AVD-SessionHost-Insights"
        Environment = "Prod"
    }
}
...
function Set-InsightsAlertsToDetailedOnly {
    ...
    $alertNameOutput = Invoke-AzCliText -Arguments @(
        ...
        "--resource-group", $ResourceGroupName,
        "--subscription", $SubscriptionId,
        "--query", "[?starts_with(name, 'AVD-SessionHost-Insights-')].name",
        "-o", "tsv"
    )
    if ([string]::IsNullOrWhiteSpace($alertNameOutput)) {
        Write-Warning "No existing AVD Session Host Insights Alerts rules were found in resource group '$ResourceGroupName'."
        return
    }
    ...
    Write-Host "All $updated AVD Session Host Insights Alerts rule(s) now use detailed-only action group '$DetailedActionGroupName'." -ForegroundColor Green
}
...
function Ensure-InsightsAlertsExist {
    ...
    if ($missingAlertNames.Count -eq 0) {
        Write-Host "All required AVD Session Host Insights Alerts rules already exist; bootstrap creation is not required." -ForegroundColor Gray
        return
    }

    $deployAlertsScript = Join-Path $PSScriptRoot "AVD-SessionHost-Insights-Category-Alerts.ps1"
    if (-not (Test-Path $deployAlertsScript)) {
        throw "Could not find AVD-SessionHost-Insights-Category-Alerts.ps1 at '$deployAlertsScript'."
    }

    Write-Host "Detected $($missingAlertNames.Count) missing AVD Session Host Insights Alerts rule(s). Bootstrapping via AVD-SessionHost-Insights-Category-Alerts.ps1..." -ForegroundColor Yellow
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Bootstrap alert creation via AVD-SessionHost-Insights-Category-Alerts.ps1 failed."
    }
    ...
    Write-Host "Bootstrap complete: required AVD Session Host Insights Alerts rules now exist." -ForegroundColor Green
}
...
if ([string]::IsNullOrWhiteSpace($CsvPath)) {
    ...
    $CsvPath = ".\avd-sessionhost-insights-logicapp-report-$subPrefix.csv"
}
...
$defaultFallbackName = "AVD-SessionHost-Insights-DefaultFallback"
...
| extend Status = case(ObjectName has 'Input Delay' and AvgValue > 200, 'CRITICAL', CounterName == 'Current TCP RTT' and AvgValue > 150, 'WARNING', CounterName == 'Current UDP Bandwidth' and AvgValue < 500, 'WARNING', 'OK')
...
@{concat(
  '<html><body style="font-family:Segoe UI,Arial,sans-serif;font-size:13px;color:#242424;">',
    '<h2 style="margin-bottom:8px;">AVD Session Host Insights Alert</h2>',
...
$sendEmailAction = @{
    ...
    inputs   = @{
        ...
        body   = @{
            To         = $SendToEmailValue
            Subject    = "@{concat('AVD Session Host Insights Alert - ', coalesce(triggerBody()?['data']?['essentials']?['alertRule'], 'Performance'))}"
...
# =========================
Write-Step "Ensuring AVD Session Host Insights Alerts exist (bootstrap if needed)"
...
Write-Step "Switching AVD Session Host Insights Alerts to detailed-only action group"
...
Write-Host "6. If session host insights alerts were missing, they were auto-created via AVD-SessionHost-Insights-Category-Alerts.ps1."
Write-Host "7. Existing session host insights alerts were switched to detailed-only action group routing."
